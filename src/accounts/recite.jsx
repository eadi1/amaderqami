...
function ReceiptPage() {
  ...
  const [currentBook, setCurrentBook] = useState({ boi_number: "", start_page: "", end_page: "",name:"" });
  ...
  const fetchBooks = async () => {
    setLoading(true);
    try {
      const data = await ApiManager.get("/receipt-book/roshid/list");
      setBooks(data);
    } catch (err) {
  ...
  const handleAdd = () => {
    setCurrentBook({name:"", boi_number: "", start_page: "", end_page: "" });
    setNextPage(null);
    setShowModal(true);
  };
  ...
  const handleNextPage = async (boi_number) => {
    ...
    try {
      const res = await ApiManager.get(`/receipt-book/roshid/next/${boi_number}`);
  ...
  const handleSave = async () => {
    const { boi_number, start_page, end_page ,name } = currentBook;
    if (!boi_number || !start_page || !end_page || !name) return toast.warning("সব ফিল্ড পূরণ করুন!");

    try {
      await ApiManager.post("/receipt-book/roshid", currentBook);
  ...
  const handleUsePage = async (boi_number) => {
    ...
    try {
      await ApiManager.post("/receipt-book/roshid/use-page", { boi_number, page_number: nextPage });
    ...
    <main className="main-container">
      ...
      <Section>
        <div className="jamat-management">
          ...
          {showModal && (
        <ModalBox
  title={currentBook.id ? "রশিদ বই সম্পাদনা করুন" : "নতুন রশিদ বই যুক্ত করুন"}
...
>
    ...
    <input
      ...
      placeholder="বই নম্বর লিখুন"
    />
  </div>
  {/* বই নম্বর */}
  <div className="modal-form-row">
    <label>বই নম্বর</label>
    <input
      type="number"
  ...
  <div className="modal-form-row">
    ...
    <input
      ...
      placeholder="শুরুর পৃষ্ঠা লিখুন"
    />
  </div>

  {/* শেষ পৃষ্ঠা */}
  <div className="modal-form-row">
    <label>শেষ পৃষ্ঠা</label>
    <input
      type="number"
      ...
      onChange={(e) =>
        setCurrentBook({
          ...currentBook,
          end_page: e.target.value,
        })
      }
      placeholder="শেষ পৃষ্ঠা লিখুন"
    />
  </div>
